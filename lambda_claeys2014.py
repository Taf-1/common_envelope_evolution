...
class Lambda:
    ...
    def compute_agb_lambda(logger: logging.Logger, mass: float, lum: float, kstar: int) -> float:
        logger.info("Computing AGB lambda using Claeys+2014 prescription")
        lambda_3 = np.minimum(-0.9, 0.58 + 0.75*np.log10(mass)) - 0.08*np.log10(lum)
        if kstar == 4:
            logger.info("Star is a CHeB, using appropriate formula")
            return 2 * 0.5
        elif kstar == 5:
            logger.info("Star is an EAGB, using appropriate formula")
            return 2 * np.minimum(0.8, np.minimum(1.25 - 0.15*np.log10(lum), lambda_3))
        else:
            logger.info("Star is a TPAGB, using appropriate formula")
    ...
    def compute_rgb_lambda(logger: logging.Logger, mass: float, menv: float, lum: float, rad: float, r_zams: float, kstar: int) -> float:
        logging.info("Computing RGB lambda using Claeys+2014 prescription")
        lambda_2 = 0.42 * (r_zams / rad) ** 0.4
        lambda_3 = np.minimum(-0.9, 0.58 + 0.75*np.log10(mass)) - 0.08*np.log10(lum)
        if menv == 0:
            logger.info("Envelope mass is zero, using lambda2")
            return 2 * lambda_2
        elif kstar == 4:
            logger.info("Envelope mass is non-zero but <= 1, using minimum of lambda2 and envelope mass")
    ...
    def compute_lambda(self) -> float:
        self.logger.info("Computing lambda using Claeys+2014 prescription")
        self.logger.info("First, checking if envelope mass > 1")
        if self.menv_1 > 1.0:
            self.logger.info("Envelope mass > 1, using AGB lambda")
            return self.compute_agb_lambda(self.logger, self.mass_1, self.lumin_1, self.kstar_1)
        else:
            self.logger.info("Envelope mass <= 1, using RGB lambda")
